...
```bash
DEBIAN_CODENAME=bookworm ./build.sh
```
...
该错误表示 XeTeX/hyperref 需要 Zapf Dingbats / Base 35 相关 TeX 字体指标，但镜像缺少推荐字体包。v1.3 已在 Dockerfile 中加入 `texlive-fonts-recommended`。如果仍使用旧镜像，请重新构建：
...
- 增加 smoke test：在有 Docker 的环境中执行 `./build.sh` 和一次真实转换。
